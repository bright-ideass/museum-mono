...
import { FaqDto } from '@core/entity/faq.entity';
...
export class FaqListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['drag', 'orderby', 'question', 'faq_type', 'language', 'CreatedTime', 'state', 'actions'];
  dataSource: MatTableDataSource<any> = new MatTableDataSource<FaqDto[]>();
  ...
  loadTable() {
    this.faqService.findAll(this.lang).subscribe(res => {
  ...
  async deleteItem(row: any) {
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res) {
        this.faqService.remove(row).subscribe(res => {
          this.toastr.info(`${row.question}`, '資料已刪除成功!');
          this.refresh();
        });
      }
    });
  ...
  dropTable(event: CdkDragDrop<any>) {
    if (event) {
      ...
      this.faqService.sort(this.dataSource.data).subscribe(res => {
        this.loadTable();
      });
    }
  }
